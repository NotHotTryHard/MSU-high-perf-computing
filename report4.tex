...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0.00298124 & 0.0126048 \\
$20{\times}20$ & 0.00594832 & 0.0170287 \\
$40{\times}40$ & 0.0130156 & 0.0340512 \\
$80{\times}80$ & 0.0326891 & 0.0773048 \\
$400{\times}600$ & 0.275843 & 0.509034 \\
$800{\times}1200$ & 1.11947 & 1.35672 \\
$4000{\times}4000$ & 57.5614 & 31.2834 \\
$6000{\times}6000$ & 175.426 & 90.8945 \\
\bottomrule
\end{tabular}
\end{table}
...
На малых сетках (до $80{\times}80$) переход от $p=1$ к $p=2$ приводит к увеличению времени из-за накладных расходов на межпроцессные коммуникации.
На крупных сетках ($4000{\times}4000$ и $6000{\times}6000$) использование двух GPU даёт существенный выигрыш: ускорение составляет $S_2\approx 1.84$ и $S_2\approx 1.93$ соответственно.
...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0.000251872 & 0.000700021 \\
$20{\times}20$ & 0.000514246 & 0.00100154 \\
$40{\times}40$ & 0.00116894 & 0.00202478 \\
$80{\times}80$ & 0.00253134 & 0.00457873 \\
$400{\times}600$ & 0.0435721 & 0.0389467 \\
$800{\times}1200$ & 0.216924 & 0.152944 \\
$4000{\times}4000$ & 14.7812 & 7.23783 \\
$6000{\times}6000$ & 45.5348 & 23.3247 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Анализ \texttt{matvec\_kernel\_time}.}
На крупных сетках матвекторное произведение является одной из доминирующих вычислительных компонент.
При переходе к $p=2$ время matvec снижается примерно в 1.85--2 раза на больших задачах, что указывает на эффективное распараллеливание вычислений между двумя GPU.
На малых сетках абсолютные времена малы, и накладные расходы на запуск ядер нивелируют выигрыш от распараллеливания.
...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0.00165698 & 0.00513512 \\
$20{\times}20$ & 0.00334263 & 0.00744419 \\
$40{\times}40$ & 0.00745579 & 0.0148964 \\
$80{\times}80$ & 0.0163257 & 0.0335859 \\
$400{\times}600$ & 0.171114 & 0.220409 \\
$800{\times}1200$ & 0.784312 & 0.653063 \\
$4000{\times}4000$ & 42.2534 & 21.3569 \\
$6000{\times}6000$ & 128.503 & 66.0983 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Анализ \texttt{vector\_kernels\_time}.}
Векторные операции (axpy, dot и т.п.) в сумме занимают значительную долю времени на крупных задачах.
На сетках $4000{\times}4000$ и $6000{\times}6000$ при $p=2$ время векторных операций снижается примерно в 1.85-1.95 раза по сравнению с $p=1$.
...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0 & 0.000893251 \\
$20{\times}20$ & 0 & 0.000551198 \\
$40{\times}40$ & 0 & 0.00122648 \\
$80{\times}80$ & 0 & 0.00299578 \\
$400{\times}600$ & 0 & 0.0330514 \\
$800{\times}1200$ & 0 & 0.0722407 \\
$4000{\times}4000$ & 0 & 0.476123 \\
$6000{\times}6000$ & 0 & 0.419498 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Анализ \texttt{halo\_mpi\_comm\_time}.}
Halo-обмены отсутствуют при \(p=1\) (один процесс не требует межпроцессных коммуникаций) и появляются при \(p=2\).
Для крупных сеток времена коммуникаций составляют менее 0.5 секунды, что является приемлемым накладным расходом относительно общего времени решения.
...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0.0000293124 & 0.000637692 \\
$20{\times}20$ & 0.0000485724 & 0.000750122 \\
$40{\times}40$ & 0.000111234 & 0.00143537 \\
$80{\times}80$ & 0.000114941 & 0.00322698 \\
$400{\times}600$ & 0.00180241 & 0.0214945 \\
$800{\times}1200$ & 0.00580618 & 0.0523098 \\
$4000{\times}4000$ & 0.108124 & 0.351376 \\
$6000{\times}6000$ & 0.199852 & 0.245889 \\
\bottomrule
\end{tabular}
\end{table}
...
Сетка & $p=1$ & $p=2$ \\
\midrule
$10{\times}10$ & 0.000748123 & 0.00280812 \\
$20{\times}20$ & 0.00151512 & 0.00444025 \\
$40{\times}40$ & 0.00312189 & 0.00860934 \\
$80{\times}80$ & 0.00954423 & 0.0194624 \\
$400{\times}600$ & 0.0440267 & 0.117045 \\
$800{\times}1200$ & 0.0830158 & 0.258518 \\
$4000{\times}4000$ & 0.318012 & 1.07856 \\
$6000{\times}6000$ & 0.820624 & 0.688954 \\
\bottomrule
\end{tabular}
\end{table}
...
При \(p=2\) добавляются регулярные копирования граничных строк в halo-обменах, из-за чего времена CPU$\leftrightarrow$GPU возрастают.
Тем не менее, на крупных задачах эти накладные расходы остаются небольшими по сравнению с выигрышем от распараллеливания вычислений.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{efficiency_all_grids.png}
    \caption{Ускорение программы на всех сетках}
    \label{fig:efficiency_all}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{efficiency_large_grids.png}
    \caption{Ускорение программы на тяжелых задачах}
    \label{fig:efficiency_large}
\end{figure}

\subsection{Результаты тестирования MPI+CUDA}
\label{sec:cuda_results}
На малых задачах (до $80{\times}80$) переход от $p=1$ к $p=2$ приводит к увеличению времени решения, что объясняется доминированием накладных расходов (инициализация, синхронизации, копирования и коммуникации) над полезными вычислениями.

Для средней сетки $800{\times}1200$ минимум \texttt{solve\_time} достигается при $p=1$; при $p=2$ накладные расходы на обмены и пересылки данных частично нивелируют выигрыш от распараллеливания.

Для крупных задач $4000{\times}4000$ и $6000{\times}6000$ использование двух GPU даёт существенное ускорение:
\begin{itemize}
  \item Сетка $4000{\times}4000$: $S_2 = T_1/T_2 = 57.56/31.28 \approx 1.84$
  \item Сетка $6000{\times}6000$: $S_2 = T_1/T_2 = 175.43/90.89 \approx 1.93$
\end{itemize}
Таким образом, на достаточно больших задачах достигается высокая эффективность распараллеливания ($E_2\approx 0.92{-}0.97$) при переходе от одного GPU к двум.
...
\paragraph{Референс для сравнения.}
В качестве референсной точки для оценки MPI ускорения возьмем результаты при $p=16$.

\begin{table}[H]
\centering
\caption{Сравнение MPI+CUDA с MPI}
\label{tab:cuda_vs_mpi}
\begin{tabular}{@{}lccc@{}}
\toprule
Сетка & $T_{\text{MPI},16}$, c & $T_{\text{CUDA},1}$, c & $T_{\text{CUDA},2}$, c \\
\midrule
$400{\times}600$  & 1.413  & 0.269 (5.3$\times$)  & 0.496 (2.8$\times$) \\
$800{\times}1200$ & 16.916 & 1.135 (14.9$\times$) & 1.376 (12.3$\times$) \\
\bottomrule
\end{tabular}
\end{table}

На обеих сетках MPI+CUDA существенно превосходит MPI-реализацию: для $400{\times}600$ достигается ускорение до 5.3$\times$, а для $800{\times}1200$ — до 14.9$\times$ (табл.~\ref{tab:cuda_vs_mpi}).

Интересно отметить, что на средних сетках минимальное время MPI+CUDA достигается при $p=1$. Это объясняется тем, что накладные расходы на halo-обмены между двумя GPU и дополнительные копирования данных превышают выигрыш от распараллеливания вычислений. Однако даже при $p=2$ MPI+CUDA значительно быстрее чистого MPI.
...
\]
\item \textbf{Независимость от числа процессов}: Решение при $p=1$ и $p=2$ совпадает с точностью машинного нуля.
\end{enumerate}
