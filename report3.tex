...
\item Требование п.6 задания (MPI+GPU не медленнее MPI/OpenMP) \textbf{выполняется только для крупной сетки} $800\times1200$, что подчёркивает необходимость правильного выбора размера задачи для GPU-ускорения.
\item Дальнейшее повышение производительности возможно через использование CUDA-aware MPI (прямые GPU$\leftrightarrow$GPU обмены), применение асинхронных CUDA-потоков и увеличение размера сетки для лучшей загрузки GPU.
\end{itemize}
